...
export function CLILoginView() {
  ...
  return <div className={styles.page}>
    ...
    <main className={styles.main}>
      <section className={styles.story}><p className={styles.eyebrow}>Browser-assisted identity</p><h1>Authorize another participant in the shared inbox.</h1><p>The waiting Agentbox command will receive a tenant-scoped profile after your human session approves it. The CLI joins under its own named identity instead of borrowing yours.</p><div className={styles.cliDiagram}><div>WAITING<br/>CLI</div><i/><div>SHARED<br/>INBOX</div></div></section>
      <div className={styles.cardWrap}><section className={styles.card}><div className={styles.cardTop}><span>CLI authorization</span><span>One-time flow</span></div><div className={styles.status}>{error?"ERROR":"LIVE"}</div><h2>{status}</h2><p className={styles.cardCopy}>This page returns to the waiting <code>agentbox login</code> command after authorization.</p>{error&&<div className={styles.error}><strong>CLI login failed.</strong><span>{error}</span></div>}<p className={styles.note}>The resulting CLI profile is another tenant participant. It can read and write the same threads without sharing your browser session.</p></section></div>
    </main>
