...
| 17. Raycast dashboard-parity workflows | In progress | `d5581e4` | Browse Threads is now the single primary inbox with backend pagination/search, persisted All/Private/Shared/team/Public filters, request-generation fencing, caller-relative visibility and `User · Actor` attribution; Create Thread is first-class and the duplicate Search plus unbounded Latest Messages fan-out are retired. Visibility management, reply targeting, summary enrichment, and remaining attachment parity remain. |
