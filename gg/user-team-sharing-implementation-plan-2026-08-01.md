...
- **Execution model:** Phases 1-19 are the shared Zodex implementation track and are complete on `feat/user-team-sharing`. The branch contains all approved code, migrations, tests, dashboard/Raycast UI, CLI, MCP, documentation, credential-free tooling, and production runbooks. Phase 20 is reserved exclusively for a credentialed local agent to perform the real PostgreSQL/R2 backup, Vercel deployment, live migration, per-user Raycast developer-mode setup, ChatGPT host-level file-attachment verification, and final production verification.
...
Checkpoint entries written before the 2026-08-03 Raycast scope extension retain the phase numbers that were current when they were recorded. Any historical reference to the former local-only Phase 15 maps to current Phase 20 and must not be used to infer current status; the table above is authoritative and now records code-bearing Phases 15-19 as complete.
...
2026-08-04 — Phase 19 / scanner readiness, host handoff, and shared-track completion
- Status: Complete
- Commit: `0ed1877`
- Implemented: Added `bun run verify:chatgpt-files` and the executable `scripts/verify-chatgpt-file-attachments.sh`. The gate rejects restored manual file-ID/path/string guidance, proves the ordinary HTTP adapter has no ChatGPT host-file type, parses the generated MCP descriptor through the dedicated `TestPostMessageFileDescriptorMatchesOpenAIContract`, and reruns structured decoding, host-artifact posting, secure downloader, R2 persistence/failure, authorization compensation, and HTTP-boundary tests. Repository CI runs the focused command before `verify:cutover`, and `verify:cutover` runs it before the full Go/dashboard/Raycast matrix. Refactored the MCP descriptor fixture into a reusable exact assertion. Updated dashboard onboarding instructions, README, raw self-host setup, AGENTS guidance, and the production cutover to require connector refresh/recreation plus Scan Tools when available and to describe artifact selection conceptually rather than through transport values. Added `docs/chatgpt-file-attachment-smoke.md`, which pins the exact deployed commit, verifies the host projection, creates a known Markdown artifact, asks ChatGPT to attach “the file I just created,” compares original/downloaded bytes and hashes, checks message/asset `User · ChatGPT` attribution and private R2 persistence, proves text-only posting, proves a literal sandbox path creates no partial state, and defines evidence/secret-handling/cleanup rules.
- Validation: `bun run verify:chatgpt-files` passed repeatedly; focused service/MCP/HTTP packages passed; `go test ./...`, `go vet ./...`, and `go build ./...` passed. `bun run verify:cutover` passed locally, including legacy/request-path guards, the new ChatGPT readiness gate, all Go tests/vet, dashboard typecheck/lint, API and CLI builds, optimized Next.js build, clean Raycast `npm ci`, all 11 zero-skip Raycast contract tests, Raycast TypeScript/native lint/build, and `git diff --check`. The local shell had no `TEST_DATABASE_URL`, so the pushed exact implementation head was also verified by GitHub Actions run `30845768752`: both `go` and `raycast` jobs completed successfully, and the `go` job ran the full gate with its PostgreSQL 16 service and `TEST_DATABASE_URL` set.
...
- Status: Complete
- Commit: `3e3ec7dd2cac1252038c0312e21e291280a634aa` (`fix: restore dashboard visibility patch proxy`).
...
2026-08-04 — Crucible follow-up / F2-F3 credential inventory and independent Raycast installations
- Status: Complete
- Commit: `12700a407edd9db5dca176755a85f71411e2409a` (`feat: add credential inventory and raycast installations`).
- Implemented: Added migration `0021_credential_inventory.sql` for persisted non-secret Raycast setup origin and credential-inventory indexes. The signed-in Credentials surface now pages active and revoked credentials together and exposes only safe metadata: stable credential ID, label, purpose, masked prefix, scopes, creation, last use, and revocation state. Rotation and revocation target the exact user-owned credential ID; revoked rows remain visible as audit history and another user receives the same not-found contract for inventory-specific actions. Added a dedicated Raycast installation creation path shared by the dashboard and CLI. It requires a distinct label, fixes `purpose=raycast`, fixes the approved `threads:read`, `threads:write`, `assets:read`, and `assets:write` scopes, returns the secret once, persists only non-secret developer-mode setup metadata, and lets the setup bundle be reopened after refresh. `agentbox raycast-key <installation-label>` now creates independent installations; `agentbox keys list`, `keys rotate <credential-id>`, and `keys revoke <credential-id>` use the same inventory. The convenient first-install onboarding flow remains available and no parallel Raycast state model was introduced.
- Observable behavior: Two Raycast installations owned by one user coexist with different credential IDs and secrets, authenticate independently, and rotating or revoking one does not affect the other installation or unrelated browser/MCP/CLI credentials. The maintained dashboard renders active and revoked history and supports exact-ID setup, rotation, and revocation. Reopened Raycast setup contains repository/path/commands/base URL/preferences/final check but never redisplays the stored secret.
- Validation: PostgreSQL 16.14 local trust-only test instance; `AGENTBOX_REQUIRE_POSTGRES_TESTS=1` and `TEST_DATABASE_URL` set. Focused verbose suites passed for service, HTTP API, CLI, PostgreSQL repository, backup/dump, and migration packages with an explicit zero-skip scan. New regression tests cover dual installation coexistence/authentication, least-privilege scopes, duplicate-label rejection, pagination, safe-list redaction, stable-ID rotation/revocation, retained revocation history, non-secret setup reopening, and cross-user isolation. `bun run test:parity`, `bun run typecheck`, `bun run lint`, and `git diff --check` passed. The test harness and cutover verifier now fail rather than skip when PostgreSQL coverage is required but its URL or dump tools are absent.
- Remaining: F4-F6 remain open. No production or credentialed host action was performed.
- Next: Replace final-key presigned writes with one coherent staged, immutable, checksum-bound direct-upload lifecycle and bounded idempotent cleanup for F4/F5.


2026-08-04 — Crucible follow-up / F4-F5 immutable staged upload lifecycle
- Status: Complete
- Commit: `f6907bbbee917c2df81cff59982acb2ea8dc52aa` (`fix: harden uploads and public reads`).
- Implemented: Replaced direct presigned writes to canonical attachment keys with a staged upload state machine in migration `0022_immutable_staged_uploads.sql`. Every upload intent now requires a lowercase hexadecimal SHA-256 digest and exact byte length. The R2 PUT signature binds `Content-Length`, `Content-Type`, the storage checksum, and AgentBox SHA-256 metadata while exposing only the staging URL and required headers to the client. Authorized finalization atomically claims the pending rows, releases PostgreSQL before R2 work, verifies staging size/MIME/checksum/metadata and source ETag, conditionally copies each object to a fresh content-addressed `agentbox/final/sha256/...` key, verifies the promoted object, then atomically inserts the message/assets and consumes the pending rows. The original capability can therefore be replayed only against staging and cannot mutate or recreate the canonical object after finalization, access loss, credential revocation, user disablement, or attachment purge. Dashboard and Raycast clients calculate SHA-256 before requesting intents; the Raycast implementation uses Web Crypto so native lint/build remain compatible.
- Observable behavior: Size, MIME, or checksum mismatches reject the whole batch without a canonical asset. Exact staging and uncommitted final-candidate keys are tracked in `upload_cleanup_objects`; rejected and expired staging objects become immediately eligible, ambiguous copies retain delayed exact-key compensation, and final candidates are cancelled transactionally only when they become canonical assets. Cleanup is bounded, repeatable, safe for missing objects, invoked opportunistically during upload traffic, and exposed through the deployment-maintenance-secret-only `POST /api/admin/uploads/cleanup?limit=N` endpoint. Stale `finalizing` claims are recovered in bounded passes after ten minutes so an unexpired upload can retry; expired claims become rejected. A user-row lock serializes the 100-active-intent quota across different threads, and the single-row repository compatibility method delegates to the same quota/cleanup path. Backup preflight inventories finalized assets separately from cleanup-tracked staging/final-candidate states.
- Validation: Focused memory/service/HTTP regressions prove immutable replay after successful finalization, team-access loss, key revocation, user disablement, and owner purge; exact-key cleanup for rejected, expired, abandoned, partially copied, and stale-finalization objects; cleanup retry/idempotence; same-length content substitution rejection; copy retry; concurrent finalization; and repeated quota abuse. PostgreSQL 16.14 regressions prove one concurrent finalization winner, stale-claim recovery/retry, and cross-thread quota serialization where exactly one ten-file batch reaches the 100-intent ceiling and the competing batch receives `UPLOAD_QUOTA_EXCEEDED`. R2 presign tests prove length/type/SHA-256 participation in SigV4. Dashboard typecheck and native Raycast typecheck/lint/distribution build pass with the checksum contract. The complete repository matrix and exact-head CI are recorded by the final integration checkpoint.
- Remaining: F6 public-read transaction/pool hardening and the final exact-head validation/CI gate. No production R2, PostgreSQL, Vercel, Raycast, or ChatGPT credential was used.
- Next: Close the anonymous public-thread snapshot before rendering, remove eager attachment HEAD requests, reauthorize exact attachment signing after storage inspection, and prove a deliberately slow asset store cannot starve unrelated operations on a tiny PostgreSQL pool.


2026-08-04 — Crucible follow-up / F6 public-read transaction and pool hardening
- Status: Complete
- Commit: `f6907bbbee917c2df81cff59982acb2ea8dc52aa` (`fix: harden uploads and public reads`).
- Implemented: Anonymous public-thread reads now load the token-authorized database snapshot, deliberately close and check that read-only transaction, and only then render the public DTO. Rendering performs zero eager R2 HEAD calls regardless of attachment count; non-purged attachments expose only token-scoped preview/download endpoint paths, and those endpoints retain exact public-token plus containing-thread authorization. Authenticated and public signing use the same two-phase boundary: acquire and close the authorization lease, inspect immutable object identity outside PostgreSQL, reacquire exact authorization immediately before local presigning, compare the asset identity, reject purge/revocation/access changes, generate the URL while the short lease is held, and deliberately report close failures.
- Observable behavior: Anonymous page reads retain no row lock or pool connection while storage is slow and issue no unbounded per-attachment storage fan-out. Missing or identity-invalid objects resolve at the bounded signing endpoint as unavailable. Public-token revocation, team/share removal, purge, or other authorization changes racing storage inspection cannot mint a fresh signed URL from a stale snapshot. Transaction-close failures are returned rather than discarded.
- Validation: Unit regressions prove zero R2 HEADs on public payload rendering, first- and second-lease close failure propagation, token revocation during storage inspection, changed/purged asset rejection, and exact reauthorization before signing. A PostgreSQL 16.14 integration test uses a two-connection pool, eight concurrent anonymous reads, and an asset store that would block for more than twelve seconds if called; login, inbox, visibility, invitation, and owner-user operations all complete within the six-second bound, while the anonymous reads complete without any storage call. The complete repository matrix and exact-head CI are recorded by the final integration checkpoint.
- Remaining: Exact-head GitHub Actions verification. Phase 20 remains reserved for credentialed production and host verification.
- Next: Push this final validation ledger, identify the workflow run for the exact resulting SHA, and do not declare completion until that exact run is green.


2026-08-04 — Crucible follow-up / final integration and zero-skip validation
- Status: Complete locally; exact-head CI pending this ledger commit.
- Implementation commit: `f6907bbbee917c2df81cff59982acb2ea8dc52aa` (`fix: harden uploads and public reads`).
- Full validation environment: PostgreSQL `16.14` on the credential-free local trust-only test instance, `TEST_DATABASE_URL` set, `AGENTBOX_REQUIRE_POSTGRES_TESTS=1`, PostgreSQL client/dump binaries on `PATH`, and an explicit scan that failed the run if any Go test emitted `SKIP`.
- Go: `git diff --check`, `go test -count=1 -v ./...`, zero-skip scan, `go vet ./...`, and `go build ./...` passed. PostgreSQL-backed coverage actually executed for migrations, backup/export/restore, credential inventory, concurrent finalization, cross-thread upload quota serialization, stale-finalization recovery, authorization leases, public-read pool behavior, and cutover postchecks.
- Dashboard/root: `bun run test:parity`, `bun run typecheck`, `bun run lint`, `bun run build:api`, `bun run build:cli`, and `bun run build` passed. The Next.js production build completed all 37 static/dynamic routes, including the canonical visibility, upload, credential, owner, and public attachment routes.
- Raycast: From `raycast/agentbox`, clean `npm ci`, `npm run test` (11/11), `npm run typecheck`, official `ray lint -I` package/icon/ESLint/Prettier validation, and official `ray build -I -e dist` passed. The checksum client uses Web Crypto rather than a Node-only crypto import so the native Raycast bundler remains compatible. The clean install reported three transitive npm audit advisories (two low, one high); package tests/lint/build remained green and dependency upgrades are outside the six acceptance blockers.
- Final gates: `bun run verify:chatgpt-files` and the hardened `bun run verify:cutover` passed. The cutover verifier independently repeated the full uncached verbose Go/PostgreSQL matrix with no skips, static analysis, dashboard builds, a clean Raycast install plus native verification, and final patch hygiene. `scripts/user-team-cutover-postcheck.sql` now requires migration `0022` and validates upload cleanup/state invariants.
- Remaining: Push this ledger-only commit and verify the required GitHub Actions workflow for its exact SHA. No Phase 20 action and no production PostgreSQL, R2, Vercel, Raycast, or ChatGPT credential was used.
- Next: Confirm a clean worktree, push normally, capture the exact head SHA and workflow run, inspect any failure narrowly, and report completion only after the exact-head run concludes successfully.


2026-08-04 — Second Crucible acceptance follow-up / F1, F3, and F5 credential and trusted-origin lifecycles
- Status: Complete.
- Commit: `4fffe879c650cf4b17f44cb4b79b4b5b8746df0d` (`fix: harden credential and origin lifecycles`).
- Implemented: Generic and onboarding credential creation are now insert-only and return `CREDENTIAL_LABEL_CONFLICT` rather than replacing an active normalized-label credential. Onboarding rotation locks and updates only the exact credential linked to that connector step; an unrelated same-label credential is rejected and a missing/revoked linked credential requires reconnect rather than implicit replacement. Raycast onboarding creation persists the trusted setup origin. Stable-ID Raycast rotation locks the credential, resolves or backfills setup metadata, constructs the complete setup result, and only then mutates the secret in the same transaction; a missing trusted origin returns `RAYCAST_SETUP_UNAVAILABLE` while the old secret remains valid. Reopening legacy setup lazily backfills the non-secret origin from the configured trusted dashboard origin without redisplaying a secret.
- Trusted-origin boundary: Production API startup now requires `AGENTBOX_APP_PUBLIC_URL` to be one absolute HTTPS origin with no userinfo, path, query, or fragment. Generated signup, owner-setup, visibility, MCP, onboarding, and Raycast setup URLs use that normalized origin. The Go server ignores `Forwarded` and `X-Forwarded-*` when deriving the development-only fallback, while the Next proxy deletes and replaces client-supplied forwarding headers with the dashboard request origin.
- Observable behavior: Repeating a create with the same case-insensitive label returns a conflict and the original secret still authenticates; secret replacement is available only through stable-ID rotation. A legacy Raycast credential with no setup metadata cannot be destructively rotated without a trusted origin, and once backfilled it can be rotated and reopened safely. A hostile forwarded host/proto cannot select a public or connector URL.
- Validation: Focused Go service, HTTP, config, and API tests passed; `bun run test:parity` passed 4 tests including forged forwarding-header replacement; `bun run typecheck`, `bun run lint`, and `git diff --check` passed. PostgreSQL `16.14` ran with `TEST_DATABASE_URL=postgres://127.0.0.1:55433/agentbox_test?sslmode=disable` and `AGENTBOX_REQUIRE_POSTGRES_TESTS=1`; focused credential inventory, user-owned credential isolation, onboarding serialization, and Raycast onboarding migration tests all executed with no skips.
- Remaining: Current-review blockers F2, F4, and F6 remain open. No production or credentialed host action was performed.
- Next: Add durable message/asset ordinals and then move authenticated, owner, and public attachment availability/signing to lazy per-asset endpoint lifecycles.


2026-08-04 — Second Crucible acceptance follow-up / F4 durable content ordering
- Status: Complete.
- Commit: `ae6d59281a8c8ccf86ab59c88f892d6fa65d3482` (`fix: persist content ordering ordinals`).
- Implemented: Added migration `0023_content_ordinals.sql`. Messages now carry a positive per-thread ordinal and assets a positive per-message ordinal, each protected by a non-null check and unique parent/position index. New-thread creation writes message position `1`; every later post allocates the next message position while holding the existing `FOR UPDATE` thread mutation lock; attachment positions are assigned from the submitted batch order in the same transaction. PostgreSQL authenticated, owner, and public detail loaders now share one position-ordered path, list/search last-message summaries use the highest ordinal, and the memory repository mirrors the same comparator and allocation rules. Internal ordinal fields are excluded from JSON, so this strengthens ordering without expanding the public DTO.
- Legacy behavior: Migration `0023` backfills the strongest recoverable deterministic order—`created_at`, then stable row ID—before making ordinals mandatory. Equal legacy timestamps do not contain enough information to reconstruct an unknowable historical submit sequence; the deterministic backfill prevents future reordering and is recorded in the Amendment below.
- Observable behavior: Concurrent posts to one thread receive unique contiguous positions. Authenticated dashboard/API, owner-content, anonymous public, CLI/MCP, and Raycast consumers receive the same message and attachment array order even when all storage timestamps are identical. Duplicate or non-positive ordinals are rejected by PostgreSQL, and the cutover postcheck now requires migration `0023` plus ordinal integrity.
- Validation: PostgreSQL `16.14` ran with `AGENTBOX_REQUIRE_POSTGRES_TESTS=1`; migration backfill/constraint tests, concurrent-post allocation, equal-timestamp authenticated/owner/public reads, legacy migration preservation, backup fixtures, and cutover postcheck passed. The complete `go test -count=1 -json ./...` suite passed with an explicit scan proving zero skipped test cases; `go vet ./...`, `go build ./...`, `bun run test:parity`, `bun run typecheck`, `bun run lint`, and `git diff --check` passed.
- Remaining: Current-review blockers F2 and F6 remain open. No production database or host action was performed.
- Next: Remove eager attachment storage inspection from authenticated and owner detail reads, make availability and signed URL resolution lazy per asset, and return a per-asset unavailable result for missing public objects without invalidating the thread.


2026-08-04 — Second Crucible acceptance follow-up / F2 and F6 lazy attachment resolution
- Status: Complete.
- Commit: `38e3417e61221813986f79724355cce027888516` (`fix: resolve attachment availability lazily`).
- Implemented: Authenticated `/api/threads/:threadId/view`, owner-browser `/api/owner/content/threads/:threadId`, and anonymous public thread reads now return safe attachment metadata plus local authorized `download_path` and image-only `preview_path` values without issuing any R2 `HEAD` or signed-URL operation. The existing `assets:read` requirement remains explicit for the authenticated attachment-enriched view. Download and preview endpoints authorize exactly one asset, inspect its stored object, reauthorize immediately before signing, and return `available:true` with one short-lived URL. A missing, size-mismatched, or checksum-invalid object returns HTTP 200 with `available:false` and one `unavailable_reason`; not-found/unauthorized assets remain indistinguishable 404s and owner-purged assets retain the explicit gone contract.
- Consumer behavior: The authenticated dashboard, read-only owner dashboard, public share UI, and Raycast client all resolve URLs only after the user selects an attachment. No UI places an endpoint path directly in an image element. A failed preview or download marks only that asset unavailable; it does not clear the thread, invalidate the public token, suppress neighboring messages, or create a broken image. Raycast continues to sign downloads lazily and maps the 200 unavailable envelope to its existing attachment-scoped coded error/tombstone flow.
- Public contract change: Authenticated and owner detail DTOs replace eager `download_url` / `preview_url` values with `download_path` / `preview_path`. Public preview changes from a redirect to the same JSON resolution envelope used by public download. Successful resolution envelopes include `available:true`; object-unavailable envelopes include `available:false` and omit the signed URL. No persistence schema or authorization predicate changed.
- Final-integration follow-up: `620e6699bc55605628e74d4d682698d491dddebe` (`fix: proxy lazy attachment previews`) adds the authenticated and owner Next.js preview proxies that the production route manifest initially exposed as missing, preserves `expires_in` on authenticated download as well as all preview/download proxies, and extends `bun run test:parity` with isolated end-to-end route forwarding tests. The rebuilt manifest now contains `/api/assets/[assetId]/preview-url` and `/api/owner/content/assets/[assetId]/preview` alongside their download routes.
- Observable behavior: Detail reads remain fast and successful even when every storage inspection would fail. Missing public objects leave the public thread readable and continue to expose only the affected attachment's authorized resolution paths; both preview and download resolve to an attachment-scoped unavailable result. Stable token revocation, cross-thread asset denial, purge tombstones, and scope checks remain unchanged.
- Validation: Focused service/HTTP tests cover authenticated, owner, and public lazy paths; forced storage-inspection failures prove both authenticated and owner detail perform zero storage calls; public tests delete the object and prove preview/download return `available:false` while repeated thread reads remain 200. The complete PostgreSQL-backed `go test -count=1 -json ./...` suite passed under PostgreSQL `16.14` and `AGENTBOX_REQUIRE_POSTGRES_TESTS=1` with zero skipped test cases; `go vet ./...`, `go build ./...`, `bun run test:parity`, `bun run typecheck`, `bun run lint`, and `git diff --check` passed. Raycast completed clean `npm ci`, 11/11 tests with zero skips, typecheck, official lint/Prettier, and native build; npm reported the same three transitive audit advisories (two low, one high) outside blocker scope.
...
### 2026-08-04 — Direct uploads finalize through checksum-bound staging

The reviewed implementation exposed that a normal presigned R2 PUT remains reusable until its expiry and therefore cannot point at the canonical attachment key. The approved direct-upload architecture is refined as follows: every client supplies exact byte length and SHA-256 before intent creation; the signed PUT is restricted to a temporary `agentbox/staging/...` key and binds length, MIME type, storage checksum, and digest metadata; authorized finalization conditionally copies the verified source ETag to a fresh content-addressed `agentbox/final/sha256/...` key; and only that final key is transactionally persisted as an asset. The database transaction is never held across R2 HEAD/copy/delete calls.

`pending_uploads` is now an explicit `pending`/`finalizing`/`finalized`/`rejected` state machine, and `upload_cleanup_objects` records every exact staging or uncommitted final-candidate key. Cleanup is bounded and idempotent, treats missing objects as success, never accepts a caller-derived prefix/key, recovers stale claims, and preserves final candidates only when the same transaction makes them canonical. Existing pre-`0022` pending rows remain preserved but have no trusted checksum identity; they are cleanup/audit state rather than valid inputs to the new finalization contract. This amendment changes no user/team authorization rule, R2 remains private, and Phase 20 must apply and verify the canonical migration before live direct uploads resume.
...
The original public-read phase described thread reads returning short-lived preview/download URLs. Acceptance evidence showed that applying that model uniformly to authenticated and owner detail caused sequential R2 `HEAD` and signing fan-out, while public preview paths were rendered directly as images and missing-object resolution could collapse the whole public thread. The approved architecture is therefore refined: thread detail returns only safe metadata and local authorized per-asset resolution paths; storage inspection and signing occur only when one preview or download is selected; and an identity-invalid or missing object produces an `available:false` result scoped to that asset. Public-token lookup, effective-access authorization, purge semantics, R2 privacy, and the pre-sign reauthorization lease remain unchanged. Phase 20 must verify live R2 missing-object and signed-preview behavior without reintroducing eager detail reads.
