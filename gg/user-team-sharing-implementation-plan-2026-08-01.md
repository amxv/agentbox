...
| 18. Raycast hardening, docs, CI, and runbook | Complete | `4756152` | Raycast now has locked package verification in repository CI, a developer-mode-only package contract, canonical dashboard onboarding and per-installation key documentation, credential-free fake smoke guidance, and an exact Phase 20 macOS/production verification runbook through migrations `0018`-`0020`. |
...
- Status: Complete
- Commit: `4756152`
- Implemented: Added a dedicated Linux CI job that uses Node.js 22, installs `raycast/agentbox/package-lock.json` with `npm ci`, and runs package-local contract tests, TypeScript, Raycast native package validation, and the production extension build. Added `npm run verify`, removed the Raycast Store publish script, and documented the extension as a reviewed developer-mode package only. Added a package README covering the final commands, dashboard-first fourth-connector onboarding, exact least-privilege scopes, one credential per installation, preference values, credential-free fake verification, rotation/revocation, and a complete macOS smoke checklist. Replaced the root README's legacy Raycast setup with the canonical onboarding flow. Extended the production cutover runbook with the post-owner migration boundary through `0018`, `0019`, and `0020`, exact macOS import commands, every normal-operation/visibility/attachment/rotation/disablement smoke check, evidence requirements, and rollback boundaries.
- Validation: Parsed `.github/workflows/ci.yml` and asserted the exact Raycast job/working directory/command sequence; asserted package scripts, root/package documentation, and the single Phase 20 runbook marker; `git diff --check`; clean `npm ci`; `CI=1 NO_COLOR=1 npm run verify` with recorded zero exit status for tests, typecheck, native lint, and build; verified the Raycast bundle exists; root `bun run typecheck` and `bun run lint` passed. No AgentBox, PostgreSQL, R2, Vercel, macOS Raycast, or production credentials were used.
- Remaining: No Phase 18 code-bearing work. Real developer-mode import, preference entry, live production smoke checks, per-installation rotation/revocation, and disposable-user disablement remain explicitly reserved for the credentialed local Phase 20 agent.
- Next: Begin Phase 19 by reading its blueprint contract and tracing the current MCP `post_message` attachment path end to end: ChatGPT Apps SDK tool schema, conversation-file identifiers, connector file parameters, backend upload/finalization, and current AgentBox MCP responses. Implement one native attachment flow that accepts ChatGPT-provided files without base64 or text chunking, preserves existing CLI/MCP callers, adds host/fake contract tests, and records any final host-level verification as Phase 20 local work.
