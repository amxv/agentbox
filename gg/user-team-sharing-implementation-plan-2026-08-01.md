...
| 17. Raycast dashboard-parity workflows | In progress | `0096cfd` | Browse Threads and Post Message now both traverse the paginated effective-access set with search and generation fencing; contextual replies compose directly, standalone replies select an accessible thread first, and manual thread IDs remain an explicit expert route. Canonical visibility management is complete. Summary enrichment and remaining attachment parity remain. |
