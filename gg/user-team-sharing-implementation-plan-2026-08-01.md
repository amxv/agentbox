...
| 19. ChatGPT-native MCP file attachments | In progress | `6262b9b` | The MCP descriptor and runtime now accept only the closed host-expanded file object, ordinary HTTP no longer accepts the structured remote-file field, manual string/URL compatibility is removed, and exact descriptor/adapter tests are in place. SSRF-safe fetching, redirect/timeout/size hardening, compensation tests, scanner readiness, and the host runbook remain. |
