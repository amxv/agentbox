...
| 6. Onboarding and connector setup | In progress | `9848372` | Persisted resumable state, browser-only onboarding APIs, independently serialized ChatGPT/Claude/local credentials, one-time connection material, local profile metadata, and setup prompt generation are implemented. The three-card onboarding dashboard and settings re-entry link remain. |
