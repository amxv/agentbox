...
- **Execution model:** Phases 1-19 are the shared Zodex implementation track. Phases 1-15 are complete; Phases 16-18 migrate the Raycast extension onto the final API and deliver ordinary-user parity, and Phase 19 repairs ChatGPT-native MCP artifact attachments. Agents working across ChatGPT sessions on `feat/user-team-sharing` must complete and push all code, migrations, tests, dashboard/Raycast UI, CLI, MCP, documentation, credential-free tooling, and production runbooks. Phase 20 is reserved exclusively for a credentialed local agent to perform the real PostgreSQL/R2 backup, Vercel deployment, live migration, per-user Raycast developer-mode setup, ChatGPT host-level file-attachment verification, and final production verification.
...
| 16. Raycast API-contract migration | In progress | `dced297` | Ordinary thread list/search now share an additive opaque keyset continuation contract backed by migration `0019`; PostgreSQL and memory traverse the same stable order for all five effective-access filters, and the dashboard consumes the page metadata with request-generation fencing and load-more behavior. Raycast DTO/client migration and package contract tests remain. |
...
- `search-threads.tsx` and `list-threads.tsx` currently provide overlapping commands, fetch only fixed-size recent/search result sets, and omit caller-relative visibility. The backend/dashboard continuation contract is complete in `dced297`; Raycast must now consume it rather than fetching a broad set and filtering authorization in memory. Rework Raycast around the unified effective-access set and All, Private, Shared with me, per-team, and Public filters. A paginated `List` with `List.Dropdown` is the natural shape.
