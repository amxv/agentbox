...
| 8. Public sharing and public page | Code complete | `0d4ddcb` | One hashed live token per thread, participant-managed lifecycle, metadata-only revisits, no-auth sanitized reads, token-scoped attachments, immediate invalidation, authenticated copy-once controls, and the noindex `/share/[token]` reader using the existing Markdown/GFM/highlighting/Mermaid renderer are implemented. |
