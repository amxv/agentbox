...
| 2. Deployment-global users and credentials | In progress | `e5dbadb` | Permanent-owner persistence plus user-owned credentials, sessions, and CLI codes are implemented through PostgreSQL, service, HTTP, and CLI layers. Owner bootstrap/recovery surfaces, invitation/user lifecycle, and the remaining tenant-era login/profile/provisioning contracts remain. |
