...
| 7. Team sharing and effective access | Code complete | `5f0b756` | Canonical thread-team shares, one owner-or-current-membership access predicate, atomic visibility APIs, immediate revocation, complete content-path authorization, the thread-page exact-set control, self-revocation handling, and caller-team/current-share preservation are implemented. |
| 8. Public sharing and public page | In progress | `1560575` | One hashed live token per thread, participant-managed create/rotate/revoke, metadata-only revisits, no-auth sanitized public reads, token-scoped attachment signing, immediate old-token invalidation, API proxies, and full backend tests are implemented. The public reader page and authenticated thread controls remain. |
...
| 11. Owner user and credential administration | In progress | `55ef9c1` | Owner-browser user listing plus disable/enable are implemented. Disablement transactionally revokes active browser sessions, user credentials, and pending CLI login codes. Owner-wide credential metadata/forced revoke and team-membership deactivation remain. |
...
2026-08-02 — Phase 7 / authenticated thread visibility control
- Status: Code complete
- Commit: `5f0b756`
- Implemented: Added a compact authenticated thread-page visibility control backed by the atomic visibility API and the caller-only team list. It displays the union of every current share and every team the caller belongs to, preserves current shares outside the caller's memberships, supports exact multi-team selection or private-only visibility, confirms dirty state, and refreshes thread access after saving. If a participant removes the final share granting their own access, the control detects the immediate 404 and returns them to the inbox while the permanent thread owner retains access. No owner-administration data or tenant selector is exposed on the thread page.
- Validation: `go test ./...`, `go vet ./...`, `bun run typecheck`, `bun run lint`, `bun run build`, and `git diff --check` passed. The optimized Next.js build includes `/threads/[id]`, `/api/threads/[id]/visibility`, and `/api/me/teams`. The Phase 7 backend tests continue to cover list/search/get/post/upload/finalize/attachment signing, membership/share changes, participant mutation, API-key non-bypass, duplicate shares, and owner persistence. PostgreSQL execution remains CI/local verification because Zodex has no database runtime.
- Remaining: No Phase 7 code-bearing work. Production migration and live multi-user browser/connector smoke checks remain Phase 15 local verification.
- Next: Begin Phase 8 by adding a single live public-token record per thread with hashed token material, atomic create/rotate/revoke/read operations, a no-auth read-only public thread route and page, attachment access constrained to the same live token, and tests proving rotation/revocation invalidate prior URLs immediately.

2026-08-02 — Phase 8 / hashed public-link lifecycle and read-only APIs
- Status: In progress
- Commit: `1560575`
- Implemented: Added canonical `thread_public_links` persistence with one row per thread, unique hashed token material, active-token indexes, creator metadata, and revocation timestamps. Any current participant may explicitly create, rotate, inspect, or revoke the live link through the same thread authorization boundary; existing live links require an explicit rotate flag and revoked links can be recreated without creating additional rows. Plaintext `agpub_…` tokens and complete public URLs are returned once, while later authenticated reads expose only safe metadata and a token prefix. Added no-auth, `no-store`, GET-only public thread and attachment routes plus Next.js proxies. Public thread payloads omit tenant IDs, owner/user/key IDs, storage keys, token hashes, and direct R2 URLs; attachment signing first proves that the requested asset belongs to the same thread as the current live token. Rotation and revocation immediately invalidate the old public thread and attachment endpoints.
- Validation: `go test ./...`, `go vet ./...`, explicit builds for `cmd/agentbox`, `cmd/api`, `cmd/migrate`, and `cmd/preflight`, `bun run typecheck`, `bun run lint`, `bun run build`, and `git diff --check` passed. Service/HTTP tests cover explicit creation, metadata-only revisits, participant management, unrelated API-key non-bypass, read-only public methods, sensitive-field omission, same-thread attachment signing, cross-thread asset denial, rotation, revocation, and recreation. PostgreSQL fixtures cover one-row-per-thread invariants, plaintext exclusion, serialized concurrent creation, token/asset lookup, rotation/revocation, and indexed query plans, but remain CI/local verification because Zodex has no database runtime.
- Remaining: Build the unauthenticated public reader page, attachment download interaction, not-found/revoked presentation, and the authenticated thread-page control for create/copy-once/rotate/revoke. Add frontend production checks, then mark Phase 8 code complete.
- Next: Add `app/public/[token]` as a read-only thread renderer backed only by `/api/public/threads/:token`; add a public-link panel to the authenticated thread page backed by `/api/threads/:id/public-link`; keep generated URLs only in current client state; then rerun the full Go/Next.js gates.
