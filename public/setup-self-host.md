...
`get_thread` remains metadata-only for attachments: it returns stable asset IDs, filenames, MIME types, and sizes without fetching R2 bytes. Agents explicitly call `read_attachment` for bounded UTF-8 Markdown/text/code reads, following `next_offset` for large files, or `download_attachment` for the original file. Downloads return one short-lived MCP resource link and transfer directly from R2 instead of proxying file bytes through the Agentbox backend. In ChatGPT, the download widget saves those directly fetched bytes into ChatGPT Library and posts a follow-up so the agent can find the exact filename and materialize it into its sandbox with Files. Agentbox does not expose a global MCP attachment resource catalog or automatically embed attachment bodies in thread reads.
