...
`get_thread` remains metadata-only for attachments: it returns stable asset IDs, filenames, MIME types, and sizes without fetching R2 bytes. Agents explicitly call `read_attachment` for bounded UTF-8 Markdown/text/code reads, following `next_offset` for large files, or `download_attachment` for the original file. Downloads return one short-lived MCP resource link plus `download_url` in structured output. MCP Apps hosts that support `ui/message` ResourceLink content can receive that same resource as a conversation user-message through the tiny download companion view; agents can still fetch `download_url` directly before `expires_in` elapses. File bytes transfer directly from R2 instead of proxying through the Agentbox backend. Agentbox does not expose a global MCP attachment resource catalog or automatically embed attachment bodies in thread reads.
