...
`get_thread` remains metadata-only for attachments: it returns stable asset IDs, filenames, MIME types, and sizes without fetching R2 bytes. Agents explicitly call `read_attachment` for bounded UTF-8 Markdown/text/code reads, following `next_offset` for large files, or `download_attachment` for the original file. Downloads return one short-lived MCP resource link plus `download_url` in structured output. The companion MCP Apps view sends the ResourceLink into the conversation when the host advertises `ui/message` ResourceLink support; in ChatGPT it otherwise uses `sendFollowUpMessage` to place the expiring `download_url` into a real follow-up turn for guarded sandbox downloaders. Other MCP Apps hosts can fall back to a text `ui/message`. File bytes transfer directly from R2 instead of proxying through the Agentbox backend. Agentbox does not expose a global MCP attachment resource catalog or automatically embed attachment bodies in thread reads.
