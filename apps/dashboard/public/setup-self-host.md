...
This guide is for a fresh deployment of the current deployment-global user/team model.
...
expiring, single-use setup token is stored.
...
`get_thread` remains metadata-only for attachments: it returns stable asset IDs, filenames, MIME types, and sizes without fetching R2 bytes. Agents explicitly call `read_attachment` for bounded UTF-8 Markdown/text/code reads, following `next_offset` for large files, or `download_attachment` for the original file. Downloads return one standard short-lived MCP resource link and transfer directly from R2 instead of proxying file bytes through the Agentbox backend. Materializing that resource into an MCP host's filesystem is host-dependent; Agentbox does not add a ChatGPT-specific file bridge. Local CLI downloads remain the canonical raw-file path and continue to fetch signed R2 URLs directly. Agentbox does not expose a global MCP attachment resource catalog or automatically embed attachment bodies in thread reads.
